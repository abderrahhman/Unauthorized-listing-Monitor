...
BATCH_SIZE = 8  # listings per batch; 4 browsers × 2 rounds each
...
def parse_listing(html, url):
    ...
    return {
        ...
        "sale_type": (p.get("offplanDetails") or {}).get("saleType"),
        "offplan_details": p.get("offplanDetails"),
...
# parallel=2: two Chrome instances work simultaneously on different listings.
# Reduced sleep times vs the original: window.state is server-rendered so 3 s
# is enough; scroll waits cut from 2 s to 1.5 s each.
@browser(parallel=1, add_arguments=["--no-sandbox", "--disable-dev-shm-usage"])
def scrape_one(driver: Driver, listing):
    """Scrapes a single listing URL. Called in parallel by botasaurus."""
    url = listing["link"]
    for attempt in range(1, 3):
        try:
            ...
            print(f"OK: {parsed.get('title', '')[:50]}", flush=True)
            return parsed
        except Exception as e:
            print(f"attempt {attempt} failed ({url[-40:]}): {type(e).__name__}", flush=True)
            driver.sleep(3)
    return None
...
def main():
    listings = json.load(open("output/bayut_listings.json"))["listings"][:NUM_LISTINGS]
    done = load_done()
    print(f"Already have {len(done)}/{NUM_LISTINGS}", flush=True)
    ...
    for batch in chunks(pending, BATCH_SIZE):
        results = scrape_one(batch)  # runs 2 in parallel, returns list
        gained = 0
        for r in results:
            if r:
                done[r["url"]] = r
        ...
        if gained == 0:
            ...
            if consecutive_batch_failures >= 3:
                print("3 batches with zero successes. Aborting to trigger restart.", flush=True)
                break
        else:
            consecutive_batch_failures = 0

    print(f"DONE: {len(done)}/{NUM_LISTINGS} listings saved to {OUT_FILE}", flush=True)
